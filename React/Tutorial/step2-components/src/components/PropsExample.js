...
function ProductCard({ name, price, discount = 0 }) {
  ...
  return (
    <div style={{ 
      ...
      <h3>{name}</h3>
      <p><strong>원가:</strong> {price.toLocaleString()}원</p>
      {discount > 0 && <p><strong>할인:</strong> {discount.toLocaleString()}원</p>}
  ...
  return (
    <div className="component-section">
      ...
      <div className="code-example">
        <strong>Props를 받는 컴포넌트:</strong><br/>
        {`function UserCard({ name, age, job, avatar }) {
  return (
    <div>
      <h3>{name}</h3>
