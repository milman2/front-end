...
// 가변 크기 리스트 아이템 컴포넌트
const VariableSizeItem: React.FC<{ index: number; style: React.CSSProperties; data: Product[] }> = ({
  index,
  style,
  data,
}) => {
...
const VirtualizationExample: React.FC = () => {
  ...
  const products = useMemo(() => {
    ...
    const productNames = ['노트북', '스마트폰', '태블릿', '헤드폰', '키보드', '마우스', '모니터', '웹캠'];
    
    return Array.from({ length: itemCount }, (_, index) => {
      const name = productNames[index % productNames.length];
      const category = categories[index % categories.length];
      const descriptionLength = Math.floor(Math.random() * 200) + 50;
      const description = `${name} 제품 설명입니다. ` + '이 제품은 고품질의 재료로 제작되었으며, 사용자에게 최고의 경험을 제공합니다. '.repeat(Math.floor(descriptionLength / 50));
  ...
  return (
    <div className="virtualization-example">
      ...
      <div className="performance-info">
        ...
        <p>렌더링되는 DOM 요소: {selectedView === 'normal' ? '100개' : '약 10-15개 (화면에 보이는 만큼)'}</p>
      ...
      <div className="explanation">
        <h3>가상화 최적화 설명</h3>
        <ul>
          <li><strong>가상화(Virtualization):</strong> 화면에 보이는 아이템만 렌더링하여 성능 최적화</li>
          <li><strong>고정 크기:</strong> 모든 아이템이 동일한 높이를 가질 때 사용</li>
          <li><strong>가변 크기:</strong> 아이템마다 다른 높이를 가질 때 사용</li>
          <li><strong>AutoSizer:</strong> 컨테이너 크기에 맞춰 가상화 리스트 크기 자동 조정</li>
          <li><strong>성능 향상:</strong> 수만 개의 아이템도 부드럽게 스크롤 가능</li>
        </ul>
      </div>
    </div>
  );
