  ...
  return (
    <div className="demo-box">
      ...
      <form onSubmit={handleSubmit} style={{ maxWidth: '400px', margin: '0 auto' }}>
        ...
        <div className="form-group">
          ...
          <input
            ...
            style={{ 
              borderColor: touched.password && errors.password ? '#dc3545' : '#ddd' 
            }}
          />
        ...
        <div className="form-group">
          ...
          <input
            ...
            style={{ 
              borderColor: touched.confirmPassword && errors.confirmPassword ? '#dc3545' : '#ddd' 
            }}
          />
      ...
      <div className="form-preview">
        ...
        <p><strong>오류 개수:</strong> {Object.keys(errors).length}개</p>
        <p><strong>터치된 필드:</strong> {Object.values(touched).filter(Boolean).length}개</p>
      </div>
    </div>
  );
}
    ...
    switch (name) {
      ...
      case 'website':
        if (value.trim() && !/^https?:\/\/.+/.test(value)) {
          newErrors.website = '올바른 URL 형식이 아닙니다. (http:// 또는 https://로 시작)';
        } else {
  ...
  return (
    <div className="demo-box">
      ...
      <form onSubmit={handleSubmit} style={{ maxWidth: '400px', margin: '0 auto' }}>
        <div className="form-group">
          ...
          <input
            ...
            style={{ 
              borderColor: touched.username && errors.username ? '#dc3545' : '#ddd' 
            }}
          />
          {validating.username && <div className="status pending">검증 중...</div>}
          {touched.username && errors.username && <div className="error">{errors.username}</div>}
          {touched.username && !errors.username && !validating.username && <div className="success">✓ 사용 가능한 사용자명입니다.</div>}
        </div>
        ...
        <div className="form-group">
          ...
          {touched.email && errors.email && <div className="error">{errors.email}</div>}
          {touched.email && !errors.email && !validating.email && <div className="success">✓ 사용 가능한 이메일입니다.</div>}
        </div>
        ...
        <div className="form-group">
          ...
          <input
            ...
            style={{ 
              borderColor: touched.website && errors.website ? '#dc3545' : '#ddd' 
            }}
          />
          {touched.website && errors.website && <div className="error">{errors.website}</div>}
        </div>
        ...
        <div style={{ textAlign: 'center', margin: '20px 0' }}>
          ...
          >
            {Object.values(validating).some(Boolean) ? '검증 중...' : 
             isFormValid ? '제출' : '입력 오류 수정 필요'}
          </button>
        </div>
      </form>
      ...
      <div className="form-preview">
        ...
        <p><strong>검증 중인 필드:</strong> {Object.values(validating).filter(Boolean).length}개</p>
        <p><strong>오류 개수:</strong> {Object.keys(errors).length}개</p>
      </div>
    </div>
  );
    ...
    <div className="demo-box">
      ...
      <div className="validation-demo">
        ...
        <div className="validation-rules">
          <ul>
            <li><strong>전화번호:</strong> 010-1234-5678 형식</li>
            <li><strong>나이:</strong> 1-120 사이의 숫자</li>
            <li><strong>약관:</strong> 반드시 동의해야 함</li>
          </ul>
        </div>
      </div>
    </div>
...
function FormValidation() {
  return (
    <div className="component-section">
      ...
      <p>
        폼 유효성 검사는 사용자 입력의 정확성을 보장하고 좋은 사용자 경험을 제공합니다.
        실시간 검증, 비동기 검증, 커스텀 검증 규칙 등을 구현해보세요.
      </p>

      <div className="code-example">
        <strong>기본 유효성 검사 패턴:</strong><br/>
    ...
    <form>
      ...
      <div className="highlight">
        ...
        <ul style={{ textAlign: 'left', display: 'inline-block' }}>
          <li><strong>실시간 검증:</strong> 사용자 입력과 동시에 검증</li>
          <li><strong>포커스 아웃 검증:</strong> 필드에서 포커스가 벗어날 때 검증</li>
          <li><strong>제출 시 검증:</strong> 폼 제출 시 전체 검증</li>
          <li><strong>비동기 검증:</strong> 서버와의 중복 체크</li>
          <li><strong>디바운스:</strong> 과도한 검증 요청 방지</li>
        </ul>
      </div>

      <div className="code-example">
        <strong>고급 유효성 검사 패턴:</strong><br/>
        {`// 1. 비동기 검증
